...
#test on other part of dataset where all don't agree
df50 = pd.read_csv("data\FinancialPhraseBank\Sentences_50Agree.txt", sep='@', header=None)
...
i = 0
for sent1 in df50[0]:
    if sent1 not in df[0].tolist():
        sents.append(sent1)
        sentiment.append(df50[1][i])
    i+=1
        
dfNew = pd.DataFrame()
dfNew['sentence'] = sents
dfNew['sentiment'] = sentiment

dfNew['pos'] = 0
dfNew['neg'] = 0
dfNew['neut'] = 0

dfNew.loc[(dfNew['sentiment'] == 'positive'), 'pos'] = 1
dfNew.loc[(dfNew['sentiment'] == 'negative'), 'neg'] = 1
dfNew.loc[(dfNew['sentiment'] == 'neutral'), 'neut'] = 1

input_idstTest2, attention_maskstTest2 = getTokens(dfNew['sentence'])

input_idstTest2 = np.reshape(np.array(input_idstTest2),(len(dfNew['sentence']),-1))
attention_maskstTest2 = np.reshape(attention_maskstTest2,(len(dfNew['sentence']),-1))

yPred = model.predict([input_idstTest2,attention_maskstTest2])
yPred = np.argmax(yPred, axis = 1)
yTrue = np.argmax(np.array(dfNew[['pos','neg','neut']]), axis=1)
